...
try:
    ...
    from django.core.url import reverse, NoReverseMatch
...
def sidebar_menu_setting():
    return getattr(settings, 'BLOG_SIDEBAR_MENU', True)
...
@register.inclusion_tag('admin/sidebar_menu.html',
                        takes_context=True)
def render_menu_app_list(context):
    ...
    if DJANGO_VERSION < (1, 8):
        ...
        implemented_engines = getattr(settings, 'ETHSHARE_ENGINES',
        ...
        if len(filtered_engines) == 0:
            raise ImproperlyConfigured(
                "blog: No compatible template engine found" +
                "blog requires one of the following engines: %s"
                % implemented_engines
            )
    ...
    if dependencie not in processors:
        raise ImproperlyConfigured(
            "blog: in order to use the 'sidebar menu' requires" +
